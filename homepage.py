...
#********************************************************     
    ...
    def account(bottom):
        ...
        def CHANGE_password(signedUser,acc_Frame,main):
            ...
            change_password_form=Frame(main,width=w,height=h-50,bg="#28394F")
            ...
            passwordLabel=Label(change_password_form,text="Password: ", font=(None,20),fg="white",bg="#28394F").pack()
            passwordEntry=Entry(change_password_form,show="*", justify="center",fg="white",bg="#1B2737",bd=0,width=40)
            passwordEntry.pack(ipady=5)

            NewpasswordLabel=Label(change_password_form,text="New Password: ", font=(None,20),fg="white",bg="#28394F").pack()
            NewpasswordEntry=Entry(change_password_form,show="*", justify="center",fg="white",bg="#1B2737",bd=0,width=40)
            NewpasswordEntry.pack(ipady=5)

            NewpasswordLabel2=Label(change_password_form,text="New Password Again: ", font=(None,20),fg="white",bg="#28394F").pack()
            NewpasswordEntry2=Entry(change_password_form,show="*", justify="center",fg="white",bg="#1B2737",bd=0,width=40)
            NewpasswordEntry2.pack(ipady=5)

            Label(change_password_form,text="",bg="#28394F").pack()
            
            Button(change_password_form,text="Enter",command=lambda:change_password(signedUser,change_password_form), font=(None,12), width=20, fg="white", bg="#1B2737", bd=0, anchor='center').pack()
            Label(change_password_form,text="",bg="#28394F").pack()
            Button(change_password_form,text="Back",command=lambda:account(change_password_form), font=(None,12), width=20, fg="white", bg="#1B2737", bd=0, anchor='center').pack()
    ...
    def loadQuiz(bottom):
        bottom.pack_forget()
        from Quiz_V1_2 import quiz
        quiz(main,root,signedUser)
...
#*******************************************************    
    ...
    leaderboard=Frame(bottom,width=w/4,height=h-80,bg="#28394F")
    ...
    revision.pack(expand=1,fill=BOTH)
...
#**********************Leaderboard***********************
    ...
    Label(leaderboard, text="Leaderboard",fg="White", bg ="#28394F",font=(None,11)).pack()
    lCardHeight=((h)/8)
    #*************************************************
    ...
    #*************************************************
#************************Buttons*************************
    Button(revision,text="Quiz",command=lambda:loadQuiz(bottom),width=150,height=12,bd=0, font=(None,20),bg="#213041",fg="white").pack()
...
#***********************Quiz Cards***********************
    ...
    with db_conn:
        ...
        curs.execute("""SELECT resultID,Score,Date 
                            ...
                            """, (signedUser,)) ##Imports the data relevant to the user
    ...
    iterations = 0
    while no_of_results>=1 or iterations < 5: ##Iteration used to prevent more than 5 previous stats being displayed
        previous_quiz_details=Frame(results,width=w/4,height=(h/7),bg="#28394F")
        previous_quiz_details.pack(expand = 1)
        previous_quiz_details.pack_propagate(0)

        user = row[iterations]
        iterations += 1
        no_of_results -= 1 ##Uses the length of the list of results to determine if the list is exhausted.

        Label(previous_quiz_details, text="Test ID: " + str(user[0]), fg="White", bg="#28394F", font=(None, 11)).pack()
        Label(previous_quiz_details, text="Users Score: " + str(user[1]), fg="White", bg="#28394F", font=(None, 11)).pack()
        Label(previous_quiz_details, text="User Percent: " + str((user[1] / 10) * 100) + "%", fg="White", bg="#28394F",font=(None, 11)).pack()
        Label(previous_quiz_details, text="Date: " + str(user[2]), fg="White", bg="#28394F", font=(None, 11)).pack()
